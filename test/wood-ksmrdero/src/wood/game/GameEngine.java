...
public class GameEngine extends Observable {
    ...
    private static final double TURNS_PER_SECOND = 100;
     ...
     *  or a player achieves the score needed to win. If either player wood.strategy throws an exception at any time,
     *  that wood.strategy will receive a score of -1 and the game will end
     */
    public void runGame() {
        ...
        try {
            ...
            // It's generally bad practice to catch generic Exceptions, but because a wood.strategy can throw an exception
    ...
    private void handleMove(WoodPlayer player, int xChange, int yChange) {
        ...
        boolean redMovingOnBlueStart = (player == redPlayer) && (nextTile.getLocation().equals(board.getBlueStartTileLocation()));
        boolean blueMovingOnRedStart = (player == bluePlayer) && (nextTile.getLocation().equals(board.getRedStartTileLocation()));
        if(redMovingOnBlueStart || blueMovingOnRedStart) {
            // Can't step on opponent's start tile
            return;
        }
    }
